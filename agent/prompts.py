...
13. **Media support**: When a function returns documents with `files` array, use the `fileUrl` field from each file object as the URL for `send_media`. Do NOT construct or guess URLs — always use the exact `fileUrl` value returned. Call `send_media` with `type: "document"` to deliver the file directly in WhatsApp.
